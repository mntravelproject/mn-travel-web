...
export function CategoriesSection({ categories }: Props) {
  ...
  return (
    <section className="max-w-[1320px] mx-auto px-6 lg:px-10 mt-32">
      <SlideUp className="mb-14 max-w-3xl">
          ...
          return (
            <StaggerItem key={c.id}>
              <Link
                ...
                className="group text-left p-6 rounded-2xl bg-[var(--cream-2)] hover:bg-[var(--ink)] hover:text-[var(--cream)] transition-all duration-500 border border-transparent hover:border-[var(--ink)] block"
              >
                <Icon
                  className="w-6 h-6 mb-12 transition-transform duration-300 group-hover:scale-110"
                  strokeWidth={1.5}
                />
